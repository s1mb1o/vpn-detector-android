...
Source: `detect/geoip/GeoIpProbes.kt`. 6 parallel probes; raw rows + derived rows.

### Per-probe rows (always INFO)
- `probe_ipify`, `probe_ipinfo`, `probe_ip-api`, `probe_ifconfig.co`, `probe_myip.com`, `probe_cf-trace`
...
| `no_phone_perm` | runtime permission state | — | denied → other consistency rows are degraded | INFO |

## Tab 4 — Probes (active network behavior)

Source: `detect/probes/ActiveProbes.kt`.

| ID | Method | FAIL when | WARN when | PASS when | Severity |
|---|---|---|---|---|---|
| `lat_ru` | Median HEAD latency to yandex.ru / mail.ru / vk.com | >200 ms | 100–200 ms | <100 ms | HARD |
| `lat_foreign` | Median HEAD latency to google / cloudflare / wikipedia | <30 ms | 30–80 ms | 80–200 ms | HARD |
| `lat_ratio` | Ordering: RU faster than foreign? | foreign faster | — | RU faster | HARD |
